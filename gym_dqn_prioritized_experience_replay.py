...
class PrioritizedReplayBuffer:
    def __init__(self, capacity: int):
        ...
        depth = int(np.ceil(np.log2(capacity)))
        self._priority_buffer = sum_tree.SumTreeQueue(depth)

    def push(self, transition: Transition):
        self._priority_buffer.push(1000)
    ...
    def sample_batch(self, batch_size: int):
        ...
        for _ in range(batch_size):
            index, priority = self._priority_buffer.sample()
            indices.append(index)
            priorities.append(priority)
        priority_info = indices, torch.tensor(priorities)
        return self._make_batch(indices), priority_info
...
if __name__ == '__main__':
    ...
    replay_buffer_size = 2**16  # 2**14 = 16384
    ...
    # Params for the prioritized replay memory
    alpha = 0.5
    beta = 0.4
        ...
        for episode in itertools.count():
            ...
            if total_steps > eps_decay_start:
                if episode % eps_decay_period == 0:
                    eps_exponent = episode // eps_decay_period
                    eps = eps_init * (eps_decay_rate ** eps_exponent)
                if eps < eps_bottom:
                    eps = eps_bottom
            ...
            for step in itertools.count():
                ...
                batch, priority_info = replay_buffer.sample_batch(batch_size)
                ...
                indices, priorities = priority_info
                weights = (1.0 / priorities) ** beta
                weights /= weights.max()
                weights /= weights.sum()
                ...
                # Update priorities
                new_priorities = losses.detach().numpy()
                for i, priority in zip(indices, new_priorities):
                    replay_buffer._priority_buffer[i] = priority ** alpha
    ...
    plt.plot(np.array(episode_losses) * 10, '.', markersize=1)
